...
Mind excludes done, completed, cancelled, archived, achieved, abandoned, discarded, and rejected records, including terminal task execution states. The query filters these before its per-kind sample limit; graph construction also excludes them before resolving canonical and legacy edges. The map, List view, category lists, counts, and saved-relationship panels all use that filtered graph. Tasks with completed, cancelled, or archived parent projects are excluded by owner-scoped parent lookups, independent of the displayed project and relationship samples. Standalone tasks and paused or waiting work remain eligible. Verified with frontend and Convex TypeScript checks and all 335 web tests; installed on the configured Convex development deployment.
