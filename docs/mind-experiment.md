...
- Styling uses Tailwind utilities and shared class strings in `mind-classes.ts`, following `page-classes.ts`. There is no separate Mind stylesheet. Mind uses a saturated category palette shared by its legend and Three.js materials. Unlit, non-tone-mapped node materials preserve those colors without lighting washing them into pastels. Selection eases node scale, opacity, and outline visibility instead of increasing brightness. Saved connections draw outward from the selected node and retract on deselection; this visual reveal does not indicate relationship direction. Transitions complete even with auto-rotation paused and become immediate with reduced motion enabled.
