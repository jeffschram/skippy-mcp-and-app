...
- Styling uses Tailwind utilities and shared class strings in `mind-classes.ts`, following `page-classes.ts`. There is no separate Mind stylesheet. Mind uses a saturated category palette shared by its legend and Three.js materials. Unlit, non-tone-mapped node materials preserve those colors without lighting washing them into pastels. Selection uses scale and an outline instead of increasing brightness.
